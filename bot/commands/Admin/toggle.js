module.exports = {
	...
	execute: async (bot, msg, args) => {
		var config = await bot.stores.configs.get(msg.guild.id); //don't resolve it to an empty object, actually,
		var mode = Math.abs((config && config.role_mode ? config.role_mode : 0) - 1);
