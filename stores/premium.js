...
class PremiumStore extends DataStore {
	...
	async create(data = {}) {
		try {
			...
			RETURNING id`,
			[data.user_id, data.sku, data.entitlement, data.perks, data.ends_at])
		} catch(e) {
			console.log(e);
	...
	async get(hid) {
		...
		if(data.rows && data.rows[0]) {
			return new Premium(this, KEYS, data.rows[0]);
		} else return new Premium(this, KEYS, { user_id: user });
	}
	...
	async getAll(user) {
		try {
			var data = await this.db.query(`SELECT * FROM premium WHERE user_id = $1`,[user]);
		} catch(e) {
			console.log(e);
			return Promise.reject(e.message);
		}
	...
	async deleteAll(user) {
		try {
			await this.db.query(`DELETE FROM premium WHERE user_id = $1`, [user]);
		} catch(e) {
			console.log(e);
			return Promise.reject(e.message);
		}
